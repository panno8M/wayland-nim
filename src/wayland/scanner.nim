import std/[strformat, xmlparser, xmltree, strutils, sequtils, parseutils, streams, deques, parseopt, algorithm]
import macros
...
usage: {PROGRAM_NAME} [OPTION] [client|server|code] [input_file output_file] [OPTION]"
...
options:
    ...
    -s,  --strict               exit immediately with an error if DTD
                                verification fails.
    -i,  --import:SYMBOL        import specified SYMBOL."""
  quit(ret)

proc scanner_version(ret: int) =
  stderr.writeLine fmt"{PROGRAM_NAME} {WAYLAND_VERSION}"
  quit(ret)
  ...
  while true:
    ...
    of cmdShortOption, cmdLongOption:
      ...
      of "s", "strict":
        ...
        next optparser
      of "i", "import":
        result.imports.add optparser.val
        next optparser
      of "e", "export":
        result.exports.add optparser.val
        next optparser
      else:
        usage(QuitFailure)
    of cmdArgument:
...
proc parse(optparser: var OptParser): Opts =
  ...
  case optparser.kind # Subcommand
  ...
  case optparser.kind # output file
  of cmdShortOption, cmdLongOption:
    usage(QuitFailure)
  of cmdArgument:
    result.output_filename = optparser.key
    if not stdout.reopen(result.output_filename, fmWrite):
      raise newException(IOError, "Could not open output file: " & result.output_filename)
    next optparser
  of cmdEnd:
    discard
